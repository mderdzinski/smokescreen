...
TanStack Query owns server-state fetching and caching. The API client in `web/src/lib/api.ts` uses typed JSON calls against the existing FastAPI `/api/*` endpoints and does not change endpoint shapes. React Router owns the root UI routes, with FastAPI preserving `/api/*`, `/old-dashboard`, and static asset paths outside the SPA fallback.
...
That starts FastAPI on `127.0.0.1:8000` via `smokescreen serve` and Vite on `127.0.0.1:5173`. Vite serves the React app at `/` and proxies `/api` and `/old-dashboard` to FastAPI.
...
FastAPI serves the React app at `/`, keeps the legacy dashboard available at `/old-dashboard`, and redirects the former `/app` mount to the equivalent root route.
